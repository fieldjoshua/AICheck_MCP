...
# AICheck v4.3.0 Universal Installer (Local Version)
...
if [ -f "./aicheck" ]; then
    ...
    if [ -d ".aicheck" ]; then
        ...
        # Create backup ignoring errors for missing files
        BACKUP_DIR=".aicheck.backup.$(date +%Y%m%d_%H%M%S)"
        mkdir -p "$BACKUP_DIR"
        
        # Copy files that exist, ignore missing ones
...
if [ "$MODE" = "update" ]; then
    ...
    if ls -d .aicheck.backup.* >/dev/null 2>&1; then
        echo -e "${BLUE}Backup created in: $(ls -dt .aicheck.backup.* | head -1)${NC}"
    fi
else
